...
LOGICAL                         :: LINPDAY             !! true  for daily input file 
...
CHARACTER(LEN=256)              :: CVNTIME             !! Netcdf VARNAME of time dimention
...
NAMELIST/NFORCE/   LINTERP, LINPEND, LINPDAY, LINPCDF, LITRPCDF, CINPMAT, DROFUNIT, &
                    ...
                    CROFCDF,CVNTIME, CVNROF,  CVNSUB,  SYEARIN,  SMONIN,SDAYIN,SHOURIN
...
LITRPCDF=.FALSE.
CINPMAT ="NONE"
DROFUNIT=86400*1000._JPRB     !! defaults input [mm/day] / DRofUnit => [m/s]
...
IF( .not. LINPCDF )THEN  !! for plain binary input, only DAILY runoff file accepted (v4.2)
  LINPDAY=.TRUE.
ENDIF

WRITE(LOGNAM,*)     "=== NAMELIST, NFORCE ==="
WRITE(LOGNAM,*)     "LINPCDF:   ", LINPCDF
WRITE(LOGNAM,*)     "LINPDAY:   ", LINPDAY
...
ENDIF
IF( .not. LINPCDF )THEN !! plain binary
  IF( LROSPLIT )THEN
...
IF( .not. LINPDAY )THEN
  IF( SYEARIN > 0 )THEN
    YYYY0=MIN(YYYY0,SYEARIN)  !! if input forcing start time is earlier than simulation time
  ENDIF
ENDIF
...
IF( .not. LINPDAY ) THEN !! only one input file during simulation period
  ...
  ENDIF
  CALL NCERROR( NF90_INQ_DIMID(ROFCDF%NCID,CVNTIME,NTIMEID),'GETTING TIME ID FORCING RUNOFF')
...
ALLOCATE( INPX(NSEQALL,INPN),INPY(NSEQALL,INPN),INPA(NSEQALL,INPN) )
...
IF(PRESENT(LECMF2LAKEC) .AND. (LECMF2LAKEC .NE. 0)) THEN
  ...
  ELSE
    !* Find levels dimension  -- (assume "time ID" appears first. CF metadata conventions)
...
IF( LINPDAY )THEN  !! for daily input file
  ...
  KMINSTA=DATE2MIN(IYYYY*10000+IMM*100+IDD,0)

  !*** 2. set file name
  WRITE(CDATE,'(I4.4,I2.2,I2.2)') IYYYY,IMM,IDD
  CIFNAME=TRIM(CROFDIR)//'/'//TRIM(CROFPRE)//TRIM(CDATE)//TRIM(CROFSUF)
  WRITE(LOGNAM,*) "CMF::FORCING_GET_CDF:",TRIM(CIFNAME), KMINSTA

  ROFCDF%CNAME=TRIM(CIFNAME)
  ROFCDF%CVAR(1)=TRIM(CVNROF)
  ROFCDF%CVAR(2)=TRIM(CVNSUB)
  IF ( .not. LROSPLIT ) THEN
    ROFCDF%CVAR(2)="NONE"
  ...
  WRITE(LOGNAM,*) "CMF::FORCING_GET_CDF: CNAME,NCID,VARID", TRIM(ROFCDF%CNAME),ROFCDF%NCID,ROFCDF%NVARID(1)
