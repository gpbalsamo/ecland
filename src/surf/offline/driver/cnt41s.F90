...
USE YOMGPD1S , ONLY: VFPGLOB, VFCLAKE, VFCLAKEF, VFLDEPTH, VFLDEPTHF, VFITM
...
ALLOCATE(ZBUFFI(NXIN,NYIN,2))
ALLOCATE(ZBUFFIAUX(NPOI,2))
        ...
        DO IST = 1, NPOI, NPROMA
          ...
          DO JL=IST,IEND
            IL = JL-IST+1
              ...
              VFLDEPTHF(IL,IBL) = VFLDEPTH(IL,IBL)
          ENDDO
        ENDDO
        !$OMP END PARALLEL DO
      ...
      ELSE 
        IF (MYPROC == 1) THEN
          IF (NDIMCDF == 2)THEN
            ...
            ZTMP = RESHAPE(ZBUFFI(:,:,2),(/NXIN*NYIN/))
            DCMFCOM(:,2) = PACK(ZTMP,LMASK(:))
          ELSE
            DO JL=1,NLALO
              JLL=INT(ZVFPGLOB(JL))
              DCMFCOM(JL,1) = ZBUFFI(JLL,1,1)
              DCMFCOM(JL,2) = ZBUFFI(JLL,1,2)
            ENDDO
          ENDIF
        ENDIF

        CALL MPL_SCATTERV(PRECVBUF=ZBUFFIAUX(:,1),KROOT=1,PSENDBUF=DCMFCOM(:,1),KSENDCOUNTS=NPOIP(:),CDSTRING="CNT41S:ZBUFFI")
        CALL MPL_SCATTERV(PRECVBUF=ZBUFFIAUX(:,2),KROOT=1,PSENDBUF=DCMFCOM(:,2),KSENDCOUNTS=NPOIP(:),CDSTRING="CNT41S:ZBUFFI")
        ...
        DO IST = 1, NPOI, NPROMA
          ...
          DO JL=IST,IEND
            IL = JL-IST+1
            IF ( VFITM(IL,IBL) > 0.01_JPRB ) THEN
              ! Update lake fraction only over land points
              VFCLAKEF(IL,IBL) = MAX(0._JPRB,MIN(0.99_JPRB,ZBUFFIAUX(JL,1)))
              VFLDEPTHF(IL,IBL) = ZBUFFIAUX(JL,2)
            ENDIF
          ENDDO
        ENDDO
        !$OMP END PARALLEL DO
        ...
        DO IST = 1, NPOI, NPROMA
          ...
          DO JL=IST,IEND
            ...
            IF ( VFITM(IL,IBL) > 0.01_JPRB ) THEN
              ! Update lake fraction only over land points
              VFCLAKEF(IL,IBL) = MAX(0._JPRB,MIN(0.99_JPRB,VFCLAKE(IL,IBL)+ZBUFFIAUX(JL,1)))
              IF (( ZBUFFIAUX(JL,1) > 0.01_JPRB ) .AND. ( ZBUFFIAUX(JL,2) > 0.05_JPRB )) THEN
                 VFLDEPTHF(IL,IBL) = ZBUFFIAUX(JL,2)
              ELSE
                 VFLDEPTHF(IL,IBL) = VFLDEPTH(IL,IBL)
              ENDIF
