...
USE YOMLUN1S , ONLY : NULOUT   ,NPOSGG   ,NPOSOCP,  NPOSFIRE, RMISS
...
USE YOMGP1S0 , ONLY : TSLNU0   ,QLINU0   ,TILNU0   ,FSNNU0   ,&
            ...
            &DFMC1NU0 ,DFMC10NU0,DFMC100NU0,DFMC1000NU0,&  ! LEFIRE
            &LLFLNU0  ,LWFLNU0  ,DFFLNU0  ,DWFLNU0            ! LEFIRE
USE YOS_FIRE_DIAG_MOD, ONLY : ZLFMCL, ZLFMCH
...
USE NCBUF_MOD, ONLY : NCVPT, NCVPT1, NCBUF_FLUSH_FILE
...
!              ---------------------
  ...
  DO JSLEV=1,NCSS
    WRITE(NULOUT,'(3(2X,E13.6))') TSLNU0(JL,JSLEV,1),&
     &QLINU0(JL,JSLEV,1),&
     &TILNU0(JL,JSLEV,1)
  ENDDO
...
!              ---------------------
  ...
  WRITE(NULOUT,'(2(2X,E13.6))') TRENU0(JL,1) , WRENU0(JL,1)
...
!              -----------------
  ...
  WRITE(NULOUT,'(E13.6)') FSNNU0(JL,1,1),TSNNU0(JL,1,1),ASNNU0(JL,1),RSNNU0(JL,1,1)
ENDIF
...
! synchronise
  ...
  IF( MYPROC == 1 ) THEN
    IF (LNCSNC) THEN
      CALL NCBUF_FLUSH_FILE(NPOS)
      CALL NCSNC(NPOS,IERR)
    ENDIF
  ENDIF
ENDIF

!--LEFIRE: fuel-moisture / fuel-load reservoirs
IF(LWRFIRE)THEN

  NPOS=NPOSFIRE

  CALL PACK_BUFFER(DFMC1NU0,SEND_BUF)
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:DFMC_1")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:DFMC_10")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:DFMC_100")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:DFMC_1000")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:LLFL")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:LWFL")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:DFFL")
  IF( MYPROC == 1 ) THEN
  ...
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:DWFL")
  IF( MYPROC == 1 ) THEN
...
! LFMC is purely diagnostic (see yos_fire_diag_mod.F90), not GP0-backed --
! packed the same way, just reading straight from the shared buffer instead
! of a GP0 slice.
  CALL PACK_BUFFER(ZLFMCL,SEND_BUF)
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:LFMC_L")
  IF( MYPROC == 1 ) THEN
    NVARID = NCVID(NPOS,'LFMC_L',IERR)
    ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
    IF(NACCUR == 1)THEN
      ZOUTPUTS(:,1)=ZOUTPUT(:,1)
      CALL NCVPT (NPOS,NVARID,ISTART3,ICOUNT3,ZOUTPUTS(:,1),IERR)
  ...
  CALL PACK_BUFFER(ZLFMCH,SEND_BUF)
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTPCDF:LFMC_H")
  IF( MYPROC == 1 ) THEN
    NVARID = NCVID(NPOS,'LFMC_H',IERR)
    ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
    IF(NACCUR == 1)THEN
      ZOUTPUTS(:,1)=ZOUTPUT(:,1)
      CALL NCVPT (NPOS,NVARID,ISTART3,ICOUNT3,ZOUTPUTS(:,1),IERR)
