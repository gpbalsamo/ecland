...
INTEGER NCID,IERR,NIDLON,NILON,NIDLAT,NILAT,NIDTIM,NITIM,NVARID,&
...
INTEGER(KIND=JPIM) :: IST,IEND,IBL
...
IF( IERR == 0 ) THEN
  ...
  DO IVAR=1,NVARS
    CALL NCVINQ(NCID,IVAR,CNAME,IDUM1,IDUM2,&
       &     IDUMAR,IDUM3,IERR)
...
! read surface geopotential        
  ...
  DO IST = 1, NPOI, NPROMA
    ...
    IBL = (IST-1)/NPROMA + 1
    DO JL=IST,IEND
      ZDZ=(VFGEO(JL-IST+1,IBL)-ZGEOPD(JL))/RG
      DO JT=1,NSTPFC
