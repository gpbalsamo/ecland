...
!        G. Balsamo   11/2025 - Add Irrigation
...
USE YOMGP1S0 , ONLY : TSLNU0   ,QLINU0   ,TILNU0   ,FSNNU0   ,&
            &TSNNU0   ,ASNNU0   ,RSNNU0   ,WRENU0   ,TRENU0   ,WTDNU0,&
...
USE YOMGPD1S , ONLY : VFZ0F    ,VFZ0H     ,VFITM    ,VFCVL &
                     &,VFCVH   ,VFCUR     ,VFIRFR  ,VFTVL  ,VFCO2TYP  , VFISOP_EP, VFTVH    ,VFSST &
                     &,VFCI    ,VFCIL     ,VFSOTY   ,VFSDOR  ,VFGEO &
                     &,VDIEWSSTL ,VDINSSSTL,VDISSHFTL &
                     &,VDIETL   ,VDTSKTL,VFLDEPTH,VFLDEPTHF,VFCLAKE,VFCLAKEF &
...
!     irrigation fraction
CALL PACK_BUFFER(VFIRFR,SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:irrfrc")
IF( MYPROC == 1 ) THEN
  NVARID = NCVID(NPOS,'irrfrc',IERR)
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
ENDIF
...
!     water-table depth (LEGWRECHARGE)
CALL PACK_BUFFER(WTDNU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:WTD")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'WTD',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('WTD',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

!     LEFIRE: dead fuel moisture / fuel load reservoirs
CALL PACK_BUFFER(DFMC1NU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:DFMC_1")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'DFMC_1',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('DFMC_1',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(DFMC10NU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:DFMC_10")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'DFMC_10',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('DFMC_10',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(DFMC100NU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:DFMC_100")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'DFMC_100',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('DFMC_100',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(DFMC1000NU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:DFMC_1000")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'DFMC_1000',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('DFMC_1000',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(LLFLNU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:LLFL")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'LLFL',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('LLFL',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(LWFLNU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:LWFL")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'LWFL',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('LWFL',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(DFFLNU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:DFFL")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'DFFL',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('DFFL',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(DWFLNU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:DWFL")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'DWFL',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('DWFL',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF

CALL PACK_BUFFER(PPRNU0, SEND_BUF)
CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:PPR")
IF( MYPROC == 1 ) THEN
  ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
  NVARID = NCVID(NPOS,'PPR',IERR)
  CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  CALL MINMAX('PPR',REAL(ZOUTPUT(:,1),KIND=JPRB),ICOUNT2(1),ICOUNT2(2),LMASK(ISTP:IENP),NULOUT)
ENDIF
...
IF (LEFLAKE) THEN
  ...
  !     flood depth
  CALL PACK_BUFFER(VFLDEPTHF,SEND_BUF)
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=SEND_BUF,KRECVCOUNTS=NPOIP(:),CDSTRING="WRTRES:LDEPTHF")
  IF( MYPROC == 1 ) THEN
    NVARID = NCVID(NPOS,'LDEPTHF',IERR)
    ZOUTPUT(:,1)=UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
    CALL NCVPT (NPOS,NVARID,ISTART2,ICOUNT2,ZOUTPUT(:,1),IERR)
  ENDIF
