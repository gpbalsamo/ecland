...
IF (LEVGEN) THEN
  CALL SURF_INQ(YDSURF=YSURF,PRDAW=RDAW)
ELSE
  CALL SURF_INQ(YDSURF=YSURF,PRDAW=RDAW,PRWSAT=RWSAT,PRWPWP=RWPWP)
ENDIF
...
!     Qrec
  IF( MYPROC == 1 ) NVARID = NCVID(NPOS,'Qrec',IERR)
  CALL PACK_BUFFER(D1SGWREC2(:,IA,:), ZVALUE)
  ZVALUE = ZVALUE*ZMM
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=ZVALUE(:),KRECVCOUNTS=NPOIP(:),CDSTRING="WRTDCDF:")
  IF( MYPROC == 1 ) THEN
    ZOUTPUT(:,1)= UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
...
!     Qcap
  IF( MYPROC == 1 ) NVARID = NCVID(NPOS,'Qcap',IERR)
  CALL PACK_BUFFER(D1SGWCAP2(:,IA,:), ZVALUE)
  ZVALUE = ZVALUE*ZMM
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=ZVALUE(:),KRECVCOUNTS=NPOIP(:),CDSTRING="WRTDCDF:")
  IF( MYPROC == 1 ) THEN
    ZOUTPUT(:,1)= UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
...
!     DelAquifer
  IF( MYPROC == 1 ) NVARID = NCVID(NPOS,'DelAquifer',IERR)
  CALL PACK_BUFFER(D1SDELAQ2(:,IA,:), ZVALUE)
  ZVALUE = ZVALUE*ZMM2
  CALL MPL_GATHERV(PRECVBUF=ZBUF(:),KROOT=1,PSENDBUF=ZVALUE(:),KRECVCOUNTS=NPOIP(:),CDSTRING="WRTDCDF:")
  IF( MYPROC == 1 ) THEN
    ZOUTPUT(:,1)= UNPACK(ZBUF(:),LMASK(ISTP:IENP),RMISS)
