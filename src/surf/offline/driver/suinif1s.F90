...
USE YOMDPHY  , ONLY : NPOI     ,NTRAC    ,NCSS, NLEV, NBLOCKS
...
DO IST = 1, NPOI, NPROMA
  ...
  IST1 = 1
  IEND1 = IEND-IST+1
  CALL SUSDP_DFLT(YDSURF=YSURF,KIDIA=IST1,KFDIA=IEND1,PTVL=VFTVL(1:IPROMA,IBL),PTVH=VFTVH(1:IPROMA,IBL),PSLT=VFSOTY(1:IPROMA,IBL),&
...
DO IST=1,NPOI,NPROMA
  ...
  IBL = (IST-1)/NPROMA + 1
  IPROMA = IEND-IST+1
  CALL SUSDP_DERIV(YDSURF=YSURF,KIDIA=IST1,KFDIA=IEND1,PSLT=VFSOTY(1:IPROMA,IBL),&
          & PSSDP2=GPD_SDP2(1:IPROMA,:,IBL),PSSDP3=GPD_SDP3(1:IPROMA,:,:,IBL),KLEVS3D=NCSS)
ENDDO
!$OMP END PARALLEL DO
