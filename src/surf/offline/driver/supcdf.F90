...
USE YOMLUN1S , ONLY : NPOSGG   ,NPOSOCP  ,NPOSFIRE ,NULOUT   ,NCTYPE
USE YOMDPHY  , ONLY : NCSS, NLAT     ,NLON, NCOM
USE YOMLOG1S , ONLY : NDIMCDF,LWRGG, LWROCP, LWRFIRE
...
!* -- 1. create netcdf file
  ...
  IF(LWRFIRE)THEN
    CALL NCERROR( NF90_CREATE('o_fire.nc',NCTYPE,NPOSFIRE),'Creating o_fire.nc file')
    WRITE(NULOUT,*)'NETCDF-FILE o_fire.nc OPENED ON UNIT ',NPOSFIRE
  ENDIF
...
!* -- END OF DEFINITIONS
  ...
  IF(LWRFIRE)THEN
    ...
    NVARS2D=10
    CVARS2D(1:NVARS2D)=(/'DFMC_1      ','DFMC_10     ','DFMC_100    ',&
                         'DFMC_1000   ','LLFL        ','LWFL        ',&
                         'DFFL        ','DWFL        ','LFMC_L      ',&
                         'LFMC_H      '/)
