...
USE SRFENE_MOD      , ONLY : SRFENE
USE SRFSN_MOD       , ONLY : SRFSN
USE SRFSN_LWIMP_MOD , ONLY : SRFSN_LWIMP
USE SRFSN_DRIVER_MOD, ONLY : SRFSN_DRIVER
USE SRFRCG_MOD      , ONLY : SRFRCG
USE SRFT_MOD        , ONLY : SRFT
USE SRFI_MOD        , ONLY : SRFI
USE SRFIL_MOD       , ONLY : SRFIL
USE SRFWL_MOD       , ONLY : SRFWL
USE SRFWEXC_MOD     , ONLY : SRFWEXC
USE SRFWEXC_VG_MOD  , ONLY : SRFWEXC_VG
USE SRFWDIF_MOD     , ONLY : SRFWDIF
USE SRFWVAPOR_MOD   , ONLY : SRFWVAPOR
USE SRFGWRECHARGE_MOD, ONLY : SRFGWRECHARGE
USE SRFWINC_MOD     , ONLY : SRFWINC
USE SRFWNG_MOD      , ONLY : SRFWNG
...
!    *PEVAPMU*      POTENTIAL EVAPORATION                     KG/M2/S
...
!    *PWTDM1M*    WATER-TABLE DEPTH (LEGWRECHARGE)                   m
...
!    *PWTDE1*     WATER-TABLE DEPTH TENDENCY (LEGWRECHARGE)        m/s
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PWTDM1M(KLON)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWTDE1(KLON) 
...
! Portion of ZSAWGFL(:,KLEVS_WB) diverted to the aquifer by SRFGWRECHARGE --
! zeroed here so it stays well-defined (and PROFD below stays unchanged)
! when LEGWRECHARGE is off.
REAL(KIND=JPRB) :: ZGWDRAINUSED(KLON)
...
CALL SRFIL(&
 ...
 & ZSLRFLTI(:,2) ,PSSRFLTI, PTSAM1M(:,1), LDLICE , LDNH   , &
 & YDCST  ,YDSOIL  ,&
...
!*         4.2b PROTOTYPE ADDITIVE SOIL WATER-VAPOUR FLUX (PHILIP & DE
!               VRIES), ON TOP OF THE LIQUID BUDGET ABOVE. OFF BY
!               DEFAULT (YDSOIL%LEWVFLUX, NAMELIST-WIRED VIA NAMPARSOIL) --
!               SEE SRFWVAPOR_MOD. USES THE SOIL TEMPERATURE ALREADY
!               UPDATED BY SRFT (SECTION 3.2) THIS TIMESTEP.

IF (YDSOIL%LEWVFLUX) THEN
  CALL SRFWVAPOR(KIDIA,KFDIA,KLON,KLEVS_WB,&
   & PTSPHY,&
   & PWSAM1M,ZTSA,&
   & PSSDP3,&
   & YDSOIL,YDCST,&
...
!*         4.2c PROTOTYPE ADDITIVE WATER-TABLE CAPILLARY-RISE RECHARGE,
!               THIRD OF THE "DEPTH TRILOGY" ALONGSIDE LEUNIFORMROOT AND
!               LEBEDROCKLIM. OFF BY DEFAULT (YDSOIL%LEGWRECHARGE, SET IN
...
CALL SRFWNG(KIDIA,KFDIA,KLON,KLEVS,KCWS,PTSPHY,KSOTY,&
 & ZWL,PWLMX,ZWSA,&
 & YDSOIL,&
 & PROFS,PROFD,PWFSD,&
 & LDLAND,PDHWLS)  

!*         4.6     CONTRIBUTION OF BOTTOM DRAINAGE TO TOTAL RUNOFF, LESS
!                  WHATEVER SRFGWRECHARGE ALREADY DIVERTED TO THE AQUIFER
!                  (ZGWDRAINUSED, ZERO WHEN LEGWRECHARGE IS OFF) --
!                  OTHERWISE THAT WATER WOULD BE COUNTED TWICE.
