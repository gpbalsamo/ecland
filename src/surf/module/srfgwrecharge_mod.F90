...
!     (2) RECHARGE (SHALLOWS THE WATER TABLE): THE ACTUAL GRAVITY-DRAINAGE
!         FLUX LEAVING THE BOTTOM SOIL LAYER, AS DIAGNOSED BY *SRFWEXC_VG*
!         (PSAWGFL/ZSAWGFL AT JK=KLEVS_WB -- ALREADY POST-LEBEDROCKLIM
!         TAPER AND POST-LEFRZFLOOR FLOOR), PASSED IN HERE AS
!         *PGWDRAINFLUX* AND DIVERTED TO THE AQUIFER IN FULL, IN PLACE OF
!         AN EARLIER PROTOTYPE'S SEPARATE, CRUDER SATURATION-THRESHOLD
!         ESTIMATE (ECLAND HAS NO EXPLICIT FIELD-CAPACITY PARAMETER TO
!         GATE ON, AND THE THRESHOLD NEEDED RECALIBRATING PER SOIL TEXTURE
!         AND PER LAYER COUNT -- SEE GIT HISTORY). USING THE REAL FLUX
!         NEEDS NO SUCH TUNING: IT IS WHATEVER SRFWEXC_VG'S OWN K(THETA)
!         AND BEDROCK TAPER ALREADY SAY IS LEAVING THE COLUMN, CONSISTENT
!         ACROSS ANY DISCRETIZATION. *PGWDRAINUSED* REPORTS HOW MUCH WAS
!         DIVERTED SO THE CALLER CAN SUBTRACT IT FROM WHAT REACHES
!         *PROFD*/QSB (LESSRO/VIC SATURATION-EXCESS RUNOFF) -- OTHERWISE
!         THE SAME WATER WOULD BE COUNTED TWICE. CONSEQUENCE FOR A WET
!         ANOMALY: QSB SHOULD NOW DROP BY (APPROXIMATELY) WHATEVER THE
!         AQUIFER GAINS, NOT SIMPLY GET A SEPARATE ADDITION ON TOP OF AN
!         UNCHANGED QSB AS THE EARLIER THRESHOLD-BASED VERSION DID.
...
REAL(KIND=JPRB) :: ZDRAIN, ZRECHARGE, ZDWTDDT
...
!*         2.    RECHARGE = THE ACTUAL GRAVITY-DRAINAGE FLUX SRFWEXC_VG
!                DIAGNOSED LEAVING THE BOTTOM LAYER, DIVERTED IN FULL.

    ZDRAIN = PGWDRAINFLUX(JL)/RHOH2O ! kg/m2/s -> m/s
    PRHSW(JL,JKBOT) = PRHSW(JL,JKBOT) - PTSPHY*ZDRAIN/RDAW(JKBOT)
    ZRECHARGE = PTSPHY*ZDRAIN ! water-equivalent depth (m)
    PGWDRAINUSED(JL) = PGWDRAINFLUX(JL)
