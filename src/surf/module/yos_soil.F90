...
LOGICAL :: LEWVFLUX ! SWITCH TO ACTIVATE ADDITIVE SOIL WATER-VAPOUR FLUX
                     ! (PHILIP & DE VRIES) -- SEE SRFWVAPOR_MOD. NAMELIST-
                     ! WIRED VIA NAMPARSOIL (RDNML_SOIL -> SUSSURF_PARAMS),
                     ! DEFAULTS TO .FALSE. (PLAIN BASELINE ECLAND) IF UNSET.
LOGICAL :: LEWPFLOOR ! SWITCH TO FLOOR THE VAN GENUCHTEN HYDRAULIC
                     ! CONDUCTIVITY/DIFFUSIVITY LOOKUP AT THE WILTING-POINT
                     ! MOISTURE VALUE (RWPWP), INSTEAD OF THE RESIDUAL
                     ! MOISTURE VALUE -- SEE SRFWEXC_VG. MATCHES THE
                     ! FLOORING ALREADY DONE BY THE LEGACY (NON-VG) SCHEME
                     ! IN SRFWEXC. NAMELIST-WIRED VIA NAMPARSOIL, SAME AS
                     ! LEWVFLUX ABOVE -- DEFAULTS TO .FALSE. IF UNSET.
LOGICAL :: LEFRZFLOOR ! SWITCH TO FLOOR THE FROZEN-FRACTION HYDRAULIC
                     ! CONDUCTIVITY (ZK/ZKM/ZKSURF) AT THE SAME WILTING-POINT
                     ! CONDUCTIVITY VALUE USED BY LEWPFLOOR, INSTEAD OF LETTING
                     ! IT BLEND TO EXACTLY ZERO AS ICE FRACTION -> 1 -- SEE
                     ! SRFWEXC_VG (THE LLFREEZ BLOCKS). SAME RATIONALE AS
                     ! LEWPFLOOR, APPLIED TO THE FREEZING-INDUCED CONDUCTIVITY
                     ! CUT-OFF RATHER THAN THE DRYNESS-INDUCED ONE. INDEPENDENT
                     ! OF LEWPFLOOR AND LEWVFLUX -- TEST SEPARATELY FIRST.
                     ! NAMELIST-WIRED VIA NAMPARSOIL, DEFAULTS TO .FALSE.
                     ! IF UNSET.
LOGICAL :: LEUNIFORMROOT ! SWITCH TO REPLACE THE DEFAULT ZENG ET AL. (1998)
                     ! DOUBLE-EXPONENTIAL ROOT FRACTION (SEE SRFROOTFR) WITH THE
                     ! UNIFORM-TO-A-MAXIMUM-DEPTH ROOT FRACTION OF STEVENS ET AL.
                     ! (2020, ATMOSPHERE, EQ. 5): Rk = MAX(0,MIN(RDMAXROOT,
                     ! Z_{K+1/2})-Z_{K-1/2}) / RDMAXROOT -- SEE SUSVEG. THEY FOUND
                     ! THIS REDUCES CHTESSEL'S UNDERESTIMATION OF EVAPORATION
                     ! UNDER WATER STRESS, TESTING RDMAXROOT IN {0.5,1,2,3} M.
                     ! HERE IT ALSO TARGETS THE FINER-DISCRETIZATION REGRESSION
                     ! SEEN WITH LEWVFLUX ETC: FINER NEAR-SURFACE LAYERS DON'T
                     ! CHANGE HOW MUCH ROOT FRACTION FALLS IN A GIVEN DEPTH BAND
                     ! (THE INTEGRAL IS THE SAME EITHER WAY), BUT THEY DO SPLIT
                     ! THAT DEMAND ACROSS SMALLER, FASTER-DEPLETING RESERVOIRS.
                     ! NAMELIST-WIRED VIA NAMPARSOIL, DEFAULTS TO .FALSE.
                     ! IF UNSET.
REAL(KIND=JPRB) :: RDMAXROOT ! MAXIMUM ROOTING DEPTH (M) FOR LEUNIFORMROOT, SEE
                     ! ABOVE. PROTOTYPE, SET IN SUSSOIL, NOT YET NAMELIST-WIRED
                     ! (UNLIKE LEUNIFORMROOT ITSELF, WHICH NOW IS).
LOGICAL :: LEFROZENSOILFIX ! SWITCH TO CAP THE PER-LAYER FROZEN FRACTION (ZF)
                     ! AT ITS DEPTH-WEIGHTED MEAN OVER THE WATER-BALANCE COLUMN
                     ! -- SEE SRFWEXC_VG. RATIONALE: WITHOUT THE CAP, A SINGLE
                     ! COLD LAYER CAN FREEZE SHUT (ZF=1) AND BLOCK INFILTRATION
                     ! FOR THE WHOLE COLUMN, DUMPING THE WATER INTO SURFACE
                     ! RUNOFF INSTEAD; PARTIALLY FROZEN SOIL STAYS PERMEABLE
                     ! VIA MACROPORES (MOHAMMED ET AL. 2019 HESS). PORTED
                     ! VERBATIM FROM IFS-SOURCE'S OWN DESTINE FIX (WHERE IT IS
                     ! UNCONDITIONAL); GATED HERE SINCE IT IS A BROAD,
                     ! UNCALIBRATED RUNOFF-PARTITION CHANGE FOR ANY SITE WITH A
                     ! COOL SEASON, NOT JUST BOREAL ONES -- SEE ITS PORTING
                     ! COMMIT FOR THE FI-HYY/AU-TUM/GF-GUY A/B EVIDENCE.
                     ! NAMELIST-WIRED VIA NAMPARSOIL, DEFAULTS TO .FALSE.
                     ! IF UNSET.
LOGICAL :: LEFIRE ! SWITCH TO RUN THE FUEL-MOISTURE/FUEL-LOAD RESERVOIR CHAIN
                     ! (LFMC_MOD, DFMC_MOD, FUEL_MOD -- PORTED FROM IFS-SOURCE'S
                     ! SPARKY FIRE-DANGER PROTOTYPE, ARPIFS/SPARKY/). LIVE FUEL
                     ! MOISTURE (LOW/HIGH VEG) IS DIAGNOSED EACH STEP FROM SOIL
                     ! MOISTURE AND LAI; DEAD FUEL MOISTURE (1H/10H/100H/1000H
                     ! TIMESCALE CLASSES) AND THE LIVE/DEAD LEAF/WOOD FUEL LOADS
                     ! ARE PROGNOSTIC, THE LATTER DRIVEN BY ECLAND'S OWN CTESSEL
                     ! NET CO2 FLUX. THE FIRE-SPREAD MODEL ITSELF WAS NEVER
                     ! IMPLEMENTED IN SPARKY (STILL A COMMENTED-OUT STUB THERE)
                     ! SO IS NOT PORTED; NOR IS ITS ATLAS/MPI NEIGHBOUR-SEARCH
                     ! MACHINERY OR ITS SATELLITE ACTIVE-FIRE INGESTION, BOTH
                     ! MEANINGLESS FOR A SINGLE-COLUMN OFFLINE SITE. WRITTEN TO
                     ! o_fire.nc WHEN LWRFIRE IS ALSO SET (NAM1S).
                     ! NAMELIST-WIRED VIA NAMPARSOIL, DEFAULTS TO .FALSE.
                     ! IF UNSET.
LOGICAL :: LEBEDROCKLIM ! SWITCH TO TAPER THE FREE-DRAINAGE (SATURATED)
                     ! HYDRAULIC CONDUCTIVITY AT THE BOTTOM SOIL LAYER TOWARDS
                     ! ZERO AS THE MODELLED PROFILE DEPTH APPROACHES THE REAL
                     ! DEPTH TO BEDROCK (RDBEDROCK) -- SEE SRFWEXC_VG. RATIONALE:
                     ! ECLAND'S BOTTOM BOUNDARY CONDITION IS FREE DRAINAGE
                     ! REGARDLESS OF WHETHER THE MODELLED COLUMN HAS ALREADY
                     ! REACHED PHYSICAL BEDROCK AT THAT SITE (E.G. SHANGGUAN ET
                     ! AL. 2017 GIVES ~2.6M AT AU-TUM, SHALLOWER THAN EVEN THE
                     ! DEFAULT 4-LAYER 2.89M PROFILE) -- WATER SHOULDN'T DRAIN
                     ! FREELY INTO SOLID ROCK. INERT WHEREVER THE MODELLED
                     ! PROFILE STAYS SHALLOWER THAN RDBEDROCK (THE COMMON CASE).
                     ! NAMELIST-WIRED VIA NAMPARSOIL, DEFAULTS TO .FALSE. IF
                     ! UNSET. RDBEDROCK IS A SINGLE GLOBAL SCALAR FOR NOW --
                     ! MAKING IT PER-GRIDPOINT (FROM THE GLOBAL BDTICM
                     ! DATASET) IS FOLLOW-UP WORK, NOT YET WIRED THROUGH.
REAL(KIND=JPRB) :: RDBEDROCK ! DEPTH TO BEDROCK (M) FOR LEBEDROCKLIM, SEE ABOVE.
                     ! FLOORED AT 1M (RZBEDROCK_MIN IN SRFWEXC_VG) BEFORE USE --
                     ! BELOW THAT, TREATED AS AN UNRELIABLE READING RATHER THAN
                     ! LET A SINGLE BAD VALUE FULLY SHUT OFF DRAINAGE.
                     ! PROTOTYPE, SET IN SUSSOIL, NOT YET NAMELIST-WIRED
                     ! (UNLIKE LEBEDROCKLIM ITSELF, WHICH NOW IS).
LOGICAL :: LEGWRECHARGE ! SWITCH TO ADD BIDIRECTIONAL WATER-TABLE/SOIL
                     ! EXCHANGE (CAPILLARY-RISE EXTRACTION, FREE-DRAINAGE
                     ! RECHARGE) -- SEE SRFGWRECHARGE_MOD. THIRD OF THE
                     ! "DEPTH TRILOGY" PROTOTYPES ALONGSIDE LEUNIFORMROOT
                     ! (ROOTING DEPTH) AND LEBEDROCKLIM (BEDROCK). THE WATER
                     ! TABLE DEPTH ITSELF IS A GENUINE, RESTART-CHECKPOINTED
                     ! PROGNOSTIC FIELD (WTDNU0/1/A IN YOMGP1S0/1/A, 'WTD' IN
                     ! THE RESTART FILE), NOT HELD HERE -- THERE IS
                     ! DELIBERATELY NO YOS_SOIL SCALAR FOR IT ANY MORE: ITS
                     ! "NO DATA AVAILABLE" DEFAULT (100M, DEEP ENOUGH TO BE
                     ! INERT EVERYWHERE, SAME RATIONALE AS RDBEDROCK'S) LIVES
                     ! ONLY WHERE THE FIELD ITSELF IS INITIALISED --
                     ! SUGP1S.F90'S COLD START AND RDSUPR.F90'S "MISSING FROM
                     ! RESTART FILE" FALLBACK -- MATCHING HOW FLAKE'S TLICE
                     ! ET AL. DEFAULT WHEN ABSENT, RATHER THAN VIA A YDSOIL
                     ! COPY THAT COULD DRIFT OUT OF SYNC WITH THEM. THE FLUX
                     ! IS BOUNDED BY THE LAYER'S OWN REFERENCE CONDUCTIVITY
                     ! AND DECAYS WITH HEIGHT ABOVE THE WATER TABLE AT THE VAN
                     ! GENUCHTEN ALPHA RATE (1/ALPHA = CHARACTERISTIC
                     ! CAPILLARY LENGTH FOR THAT TEXTURE) -- NO NEW TUNABLE
                     ! CONSTANTS BEYOND THE AQUIFER SIZE ITSELF (RGWSPECYIELD,
                     ! BELOW). NAMELIST-WIRED VIA NAMPARSOIL, DEFAULTS TO
                     ! .FALSE. IF UNSET.
REAL(KIND=JPRB) :: RWTDRECHARGE ! FRACTION OF THE VIC-PARTITIONED BOTTOM
                     ! DRAINAGE THAT IS ALLOWED TO REACH THE AQUIFER AS
                     ! RECHARGE (DIMENSIONLESS 0-1), APPLIED ON TOP OF THE
                     ! STATE-DEPENDENT PARTITION IN SRFGWRECHARGE_MOD. THE
                     ! REMAINDER STAYS LATERAL FLOW (PROFD/QSB), SO THE WATER
                     ! BALANCE CLOSES WITHOUT ANY EXTRA BOOKKEEPING.
                     !
                     ! WHY IT EXISTS: THE VIC PARTITION ALONE REDUCES TO
                     ! (WTD/RGWLATDEPTH)**(ZBWS/(1+ZBWS)), AND ZBWS IS CLAMPED
                     ! TO [0.01,0.5], SO THAT EXPONENT NEVER EXCEEDS 1/3 -- THE
                     ! RECHARGE FRACTION IS 0.65-0.99 ACROSS THE WHOLE LEGAL
                     ! RANGE OF THE SHAPE PARAMETER AT A 27 M WATER TABLE.
                     ! ESSENTIALLY ALL BOTTOM DRAINAGE BECOMES RECHARGE, AND
                     ! SINCE CAPILLARY-RISE EXTRACTION (THE ONLY SINK) IS
                     ! ~1E-10 AT THOSE DEPTHS, NOTHING BALANCES IT: THE WATER
                     ! TABLE FREE-FALLS AT -RECHARGE/RGWSPECYIELD TOWARD THE
                     ! RGWTD_MIN FLOOR. THIS IS A DIRECT RATE CONTROL ON THAT
                     ! COLLAPSE, ACTING IN THE REGIME WHERE THE VIC TERM IS
                     ! SATURATED AND RGWLATSIGMIN/RGWLATSIGMAX HAVE NO PURCHASE
                     ! (SEE 179176E'S FOUR-REGIME SWEEP, ALL 16/29).
                     !
                     ! IT SCALES A RATE, NOT AN EQUILIBRIUM: IT SLOWS THE
                     ! COLLAPSE RATHER THAN STOPPING IT, SO A CALIBRATED VALUE
                     ! IS TIED TO THE LENGTH OF RECORD IT WAS TUNED AGAINST.
                     ! THE STRUCTURAL FIX IS LATERAL AQUIFER TRANSPORT (PLANNED
                     ! VIA CAMA-FLOOD ADVECTION), NOT A LARGER 1D SINK.
                     ! DEFAULTS TO 1.0 = THE PRE-EXISTING BEHAVIOUR, BIT FOR
                     ! BIT. NAMELIST-WIRED VIA NAMPARSOIL.
REAL(KIND=JPRB) :: RGWSPECYIELD ! AQUIFER SPECIFIC YIELD (DRAINABLE POROSITY,
                     ! DIMENSIONLESS 0-1) FOR LEGWRECHARGE'S PROGNOSTIC WATER
                     ! TABLE -- SEE SRFGWRECHARGE_MOD. SETS THE SIZE OF THE
                     ! "BUCKET": WATER TABLE DEPTH CHANGES BY (EXTRACTION MINUS
                     ! RECHARGE THIS TIMESTEP)/RGWSPECYIELD, THE SAME
                     ! STORAGE-TO-DEPTH RELATIONSHIP AS NIU ET AL. (2007)'S
                     ! SIMPLE GROUNDWATER SCHEME FOR CLM. AN HONEST GUESS, NOT
                     ! A CALIBRATION: 0.1 SITS IN THE MIDDLE OF THE TYPICAL
                     ! LITERATURE RANGE FOR UNCONFINED AQUIFERS (~0.01-0.35
                     ! DEPENDING ON MATERIAL). PROTOTYPE, SET IN SUSSOIL, NOT
                     ! YET NAMELIST-WIRED (UNLIKE LEGWRECHARGE ITSELF, WHICH
                     ! NOW IS) OR PER-GRIDPOINT (E.G. FROM SOIL TEXTURE OR A
                     ! GLOBAL SPECIFIC-YIELD MAP).
LOGICAL :: LESSRO   ! SWITCH TO ACTIVATE SUB-GRID SURFACE RUNOFF
LOGICAL :: LEIRRIGATION   ! SWITCH TO ACTIVATE IRRIGATION
