...
LOGICAL :: LEVAPSOIL ! SWITCH TO ACTIVATE ADDITIVE SOIL WATER-VAPOUR FLUX
                     ! (PHILIP & DE VRIES) -- SEE SRFWVAPOR_MOD. PROTOTYPE,
                     ! DEFAULTS TO .FALSE. IN SUSSOIL, NOT YET NAMELIST-WIRED.
LOGICAL :: LEWPFLOOR ! SWITCH TO FLOOR THE VAN GENUCHTEN HYDRAULIC
                     ...
                     ! IN SRFWEXC. PROTOTYPE, DEFAULTS TO .FALSE. IN SUSSOIL,
                     ! NOT YET NAMELIST-WIRED.
LOGICAL :: LEFRZFLOOR ! SWITCH TO FLOOR THE FROZEN-FRACTION HYDRAULIC
                     ...
                     ! OF LEWPFLOOR AND LEVAPSOIL -- TEST SEPARATELY FIRST.
                     ! PROTOTYPE, DEFAULTS TO .FALSE. IN SUSSOIL, NOT YET
                     ! NAMELIST-WIRED.
LOGICAL :: LEUNIFORMROOT ! SWITCH TO REPLACE THE DEFAULT ZENG ET AL. (1998)
                     ...
                     ! SEEN WITH LEVAPSOIL ETC: FINER NEAR-SURFACE LAYERS DON'T
...
LOGICAL :: LEBEDROCKLIM ! SWITCH TO TAPER THE FREE-DRAINAGE (SATURATED)
                     ...
                     ! PROTOTYPE, DEFAULTS TO .FALSE. IN SUSSOIL, NOT YET
                     ! NAMELIST-WIRED. RDBEDROCK IS A SINGLE GLOBAL SCALAR FOR
                     ! NOW -- MAKING IT PER-GRIDPOINT (FROM THE GLOBAL BDTICM
...
LOGICAL :: LEGWRECHARGE ! SWITCH TO ADD A GARDNER (1958)-TYPE EXPONENTIAL
                     ! CAPILLARY-RISE RECHARGE FROM THE WATER TABLE (RWTD)
                     ! INTO EVERY SOIL LAYER ABOVE IT -- SEE
                     ! SRFGWRECHARGE_MOD. THIRD OF THE "DEPTH TRILOGY"
                     ! PROTOTYPES ALONGSIDE LEUNIFORMROOT (ROOTING DEPTH) AND
                     ! LEBEDROCKLIM (BEDROCK). THE FLUX IS BOUNDED BY THE
                     ! LAYER'S OWN SATURATED CONDUCTIVITY AND DECAYS WITH
                     ! HEIGHT ABOVE THE WATER TABLE AT THE VAN GENUCHTEN ALPHA
                     ! RATE (1/ALPHA = CHARACTERISTIC CAPILLARY LENGTH FOR
                     ! THAT TEXTURE) -- NO NEW TUNABLE CONSTANTS, JUST
                     ! PARAMETERS ECLAND ALREADY COMPUTES. PURELY ADDITIVE
                     ! LIKE LEVAPSOIL: NO NEW PROGNOSTIC STATE, DOESN'T TOUCH
                     ! THE IMPLICIT PCFW MATRIX. PROTOTYPE, DEFAULTS TO
                     ! .FALSE. IN SUSSOIL, NOT YET NAMELIST-WIRED.
REAL(KIND=JPRB) :: RWTD ! WATER TABLE DEPTH (M) FOR LEGWRECHARGE, SEE ABOVE.
                     ! SINGLE GLOBAL SCALAR FOR NOW, NOT YET PER-GRIDPOINT.
                     ! LIKE RDBEDROCK, THERE'S NO WAY TO DERIVE A MEANINGFUL
                     ! WATER TABLE DEPTH FROM QUANTITIES ECLAND ALREADY HAS --
                     ! IT'S A GENUINELY EXTERNAL PHYSICAL FACT (E.G. FAN ET
                     ! AL. 2017). "NO DATA AVAILABLE" DEFAULT IS THEREFORE THE
                     ! SAME KIND OF CHOICE AS RDBEDROCK'S: DEEP ENOUGH TO BE
                     ! INERT EVERYWHERE (100M), NOT A GUESSED SHALLOW VALUE.
                     ! PROTOTYPE, SET IN SUSSOIL, NOT YET NAMELIST-WIRED. RWTD
                     ! IS ALSO NOW ONLY THE *INITIAL* WATER TABLE DEPTH: SEE
                     ! RGWSPECYIELD BELOW -- THE ACTUAL PROGNOSTIC DEPTH USED
                     ! AT RUNTIME LIVES IN SRFGWRECHARGE_MOD, NOT HERE.
REAL(KIND=JPRB) :: RGWSPECYIELD ! AQUIFER SPECIFIC YIELD (DRAINABLE POROSITY,
                     ...
                     ! "BUCKET": WATER TABLE DEPTH DEEPENS BY (CAPILLARY-RISE
                     ! EXTRACTION THIS TIMESTEP)/RGWSPECYIELD, THE SAME
                     ...
                     ! YET NAMELIST-WIRED OR PER-GRIDPOINT (E.G. FROM SOIL
                     ! TEXTURE OR A GLOBAL SPECIFIC-YIELD MAP).
