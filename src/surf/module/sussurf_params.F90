...
! "Depth trilogy" prototype switches (see yos_soil.F90) -- namelist-set here,
! ahead of SUSSOIL's own call in SUSURF_CTL, so SUSSOIL must not overwrite
! these (it no longer does; see the note there).
YDSOIL%LEWVFLUX        = TMP_SURF%LEWVFLUX
YDSOIL%LEWPFLOOR       = TMP_SURF%LEWPFLOOR
YDSOIL%LEFRZFLOOR      = TMP_SURF%LEFRZFLOOR
YDSOIL%LEUNIFORMROOT   = TMP_SURF%LEUNIFORMROOT
YDSOIL%LEBEDROCKLIM    = TMP_SURF%LEBEDROCKLIM
YDSOIL%LEGWRECHARGE    = TMP_SURF%LEGWRECHARGE
YDSOIL%RWTDRECHARGE    = TMP_SURF%RWTDRECHARGE
YDSOIL%LEFROZENSOILFIX = TMP_SURF%LEFROZENSOILFIX
YDSOIL%LEFIRE          = TMP_SURF%LEFIRE
...
YDFLAKE%RDEPTH_W_MIX_IW        = TMP_SURF%RDEPTH_W_MIX_IW
YDFLAKE%TMNW_NDG_TIMESCL       = TMP_SURF%TMNW_NDG_TIMESCL
YDFLAKE%TMNW_NDG_RDSCL         = TMP_SURF%TMNW_NDG_RDSCL  
YDFLAKE%RDEPTH_BOTSED          = TMP_SURF%RDEPTH_BOTSED
YDFLAKE%RPHI_BOTSED_PR0        = TMP_SURF%RPHI_BOTSED_PR0
YDFLAKE%LFLAKE_BOTSED          = TMP_SURF%LFLAKE_BOTSED
